...
export class MainMenu extends Scene
{
    ...
    createMenu(menuEntries: string[]): void {
        ...
        for (let i = 0;i < menuEntries.length; i++) {
            ...
            item.on(Input.Events.POINTER_OVER, function(this: MainMenu) : void {        // set event action for mouse over (selecting it)
                this.selectSpecific(i);
            }, this);

            item.on(Input.Events.POINTER_DOWN, function(this: MainMenu) : void {        // set event action for pointer down (clicking it with the mouse)
    ...
    addKeys(): void {
        ...
        this.input.keyboard!.addKey('Down').on('down', function(this: MainMenu) { this.selectNext() }, this);
        this.input.keyboard!.addKey('S').on('down', function(this: MainMenu) { this.selectNext() }, this);
        this.input.keyboard!.addKey('Up').on('down', function(this: MainMenu) { this.selectPrevious() }, this);
        this.input.keyboard!.addKey('W').on('down', function(this: MainMenu) { this.selectPrevious() }, this);

        // enter and space key (confirming a selection)
        this.input.keyboard!.addKey('Enter').on('down', function(this: MainMenu) { this.spaceEnterKey() }, this);
        this.input.keyboard!.addKey('Space').on('down', function(this: MainMenu) { this.spaceEnterKey() }, this);
